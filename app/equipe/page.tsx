...
type FormularioUsuario = {
  ...
  ativo: boolean;
  foto: string;
  valorPorDigitacao: string;
  bonusMeioMilhao: string;
  bonusUmMilhao: string;
  metaCoordenacao: string;
  percentualCoordenacaoAbaixo: string;
  percentualCoordenacaoAcima: string;
};
...
const formularioVazio: FormularioUsuario = {
  ...
  ativo: true,
  foto: "",
  valorPorDigitacao: "10",
  bonusMeioMilhao: "250",
  bonusUmMilhao: "500",
  metaCoordenacao: "300000",
  percentualCoordenacaoAbaixo: "1",
  percentualCoordenacaoAcima: "2",
};
...
async function salvar(
  ...
  function editar(usuario: Usuario) {
    ...
    setForm({
      ...
      ativo: usuario.ativo,
      foto: usuario.foto || "",
      valorPorDigitacao: String(usuario.valorPorDigitacao ?? 10),
      bonusMeioMilhao: String(usuario.bonusMeioMilhao ?? 250),
      bonusUmMilhao: String(usuario.bonusUmMilhao ?? 500),
      metaCoordenacao: String(usuario.metaCoordenacao ?? 300000),
      percentualCoordenacaoAbaixo: String(usuario.percentualCoordenacaoAbaixo ?? 1),
      percentualCoordenacaoAcima: String(usuario.percentualCoordenacaoAcima ?? 2),
    });
    ...
    <option value="Administrativo / RH">
      Administrativo / RH
            ...
            {form.perfil === "Operacional" && (
              <>
                <label>
                  Valor por digitação paga
                  <input type="number" min="0" step="0.01" value={form.valorPorDigitacao} onChange={(evento) => setForm({ ...form, valorPorDigitacao: evento.target.value })} />
                </label>
                <label>
                  Bônus ao atingir R$ 500 mil
                  <input type="number" min="0" step="0.01" value={form.bonusMeioMilhao} onChange={(evento) => setForm({ ...form, bonusMeioMilhao: evento.target.value })} />
                </label>
                <label>
                  Bônus ao atingir R$ 1 milhão
                  <input type="number" min="0" step="0.01" value={form.bonusUmMilhao} onChange={(evento) => setForm({ ...form, bonusUmMilhao: evento.target.value })} />
                </label>
              </>
            )}

            {form.perfil === "Coordenadora" && (
              <>
                <label>
                  Meta para mudar percentual
                  <input type="number" min="0" step="1000" value={form.metaCoordenacao} onChange={(evento) => setForm({ ...form, metaCoordenacao: evento.target.value })} />
                </label>
                <label>
                  Comissão abaixo da meta (%)
                  <input type="number" min="0" step="0.01" value={form.percentualCoordenacaoAbaixo} onChange={(evento) => setForm({ ...form, percentualCoordenacaoAbaixo: evento.target.value })} />
                </label>
                <label>
                  Comissão na meta ou acima (%)
                  <input type="number" min="0" step="0.01" value={form.percentualCoordenacaoAcima} onChange={(evento) => setForm({ ...form, percentualCoordenacaoAcima: evento.target.value })} />
                </label>
              </>
            )}
                ...
                <div className="user-main">
                  ...
                  <div>
                    ...
                    {usuario.perfil === "Operacional" && (
                      <b>R$ {Number(usuario.valorPorDigitacao || 0).toFixed(2).replace(".", ",")} / digitação</b>
                    )}
                  </div>
                </div>
